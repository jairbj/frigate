"""Peewee migrations -- 036_add_recording_stream.py.
...
def migrate(migrator, database, fake=False, **kwargs):
    ...
    logger.info("Creating index on recordings(camera, stream, start_time)...")
    migrator.sql(
        'CREATE INDEX IF NOT EXISTS "recordings_camera_stream_start_time" '
        'ON "recordings" ("camera", "stream", "start_time")'
    )
    logger.info("Finished creating index on recordings(camera, stream, start_time).")
